...
postgresql etc. (except Iteration 6). If it does require such programs,
...
* You have to write a file with documentation. Everything must be documented: how to run scripts, how to run tests, how to install the library etc.

## [Iteration 1] One-shot command-line RSS reader.
RSS reader should be a command-line utility which receives [RSS](https://en.wikipedia.org/wiki/RSS) URL and prints results in human-readable format.
...
7. Make sure that your app **has no encoding issues** (meaning symbols like `&#39` etc.) when printing news to _stdout_.
8. Make sure that your app **has no encoding issues** (meaning symbols like `&#39` etc.) when printing news to _stdout in JSON format_.
9. It is preferable to have different custom exceptions for different situations(If needed).
...
meaning that this should work:
    ...
    > rss_reader ...
    ```
2. Make sure your second iteration works on a clean machine with python 3.9. (!)
...
1. Try to make your application cross-platform, meaning that it should work on both Linux and Windows.
...
New optional argument must be added to your utility. This argument receives the path where new file will be saved. The arguments should represent which format will be generated.
